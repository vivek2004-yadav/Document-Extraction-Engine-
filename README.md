...
### 1. The React Frontend (inside frontend/src/)
...
### 2. The Python Backend (inside backend/app/)
...
* **extractor.py**: Integrates with LLMs (Google Gemini and OpenAI) with strict temperature control to prevent guesswork. If a field is missing in the document, it safely returns a null value along with an explanation note.
